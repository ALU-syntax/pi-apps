			...
			$.post("modules/approval/proses.php",
				...
				function(result,status){  // ketika sukses menyimpan data
					...
					if (data.error) {
						...
						//var table = $('#dataTables3').DataTable(); 
						//table.ajax.reload( null, false );					
						window.location.href = '?module=form_approval&form=add&kode=<?= $kode_transaksi ?>'; 
					}
...
if ($_GET['form']=='add') { ?> 
  ...
  <section class="content-header">
    <h1>
      <i class="fa fa-edit icon-title"></i> Input Data Approval
    </h1>
    <ol class="breadcrumb">
      <li><a href="?module=beranda"><i class="fa fa-home"></i> Beranda </a></li>
      <li><a href="?module=approval"> Approval </a></li>
        ...
        <div class="box box-primary">
          ...
          <form role="form" class="form-horizontal" name="formPartKeluar" id="formPartKeluar">  <!--action="modules/part-masuk/proses.php?act=insert" method="POST" --> 
			...
			<div class="box-footer">
				...
				$query = mysqli_query($mysqli, "SELECT a.id, a.kode_item, b.nama_part AS nama_item, a.qty, b.satuan
				...
				while ($data = mysqli_fetch_assoc($query)) { 
					...
					echo "<tr>
						...
						<td width='50' align='right'><input type='number' value='$data[qty]' class='form-control' onchange=\"updateRow($data[id], this.value)\"></td>
						<td width='50' class='center'>$data[satuan]</td>
						<td class='center' width='30'><a class='btn btn-danger btn-xs' onclick='removeRow($data[id])'><i class='glyphicon glyphicon-trash'></i></a> 
							</a>
						</td>
						</tr>";
					$no++;
